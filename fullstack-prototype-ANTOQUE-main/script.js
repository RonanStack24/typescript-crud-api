...
document.addEventListener("DOMContentLoaded", async () => {
  loadFromStorage();
  await checkAuthToken(); // Check for saved token before route handling
  handleRouting();

  // Attach login form listener after DOM is ready
...
// Check if token exists and update UI based on role
  ...
  if (token) {
    try {
      ...
      if (response.ok) {
        ...
        // Update UI based on role (admin-only buttons, etc.)
